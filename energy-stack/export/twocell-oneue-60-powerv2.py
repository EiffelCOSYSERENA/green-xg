...
def plot_comparison(df1, df2):
    ...
    axes[1].annotate(f"{e1:.0f} J\n{t_str}", xy=(t_end, e1), xytext=(10, 0),
                     textcoords="offset points", color="tab:blue", fontsize=9,
                     arrowprops=dict(arrowstyle="->", color="tab:blue"))
    axes[1].annotate(f"{e2:.0f} J\n{t_str}", xy=(t_end, e2), xytext=(10, -25),
                     textcoords="offset points", color="tab:orange", fontsize=9,
...
if __name__ == "__main__":
    start = int(datetime.datetime(2025, 9, 29, 8, 50).timestamp())
    end   = int(datetime.datetime(2025, 9, 29, 9, 50).timestamp())
    ...
    merged.to_csv("cells_metrics.csv")
